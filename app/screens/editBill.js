...
import {
  deleteeditqrdataAction,
  editSetqrdataAction,
  geteditqrdataAction,
} from '../redux/actions/editBillAction';

const editBill = ({navigation}) => {
  ...
  const onchange = async text => {
    ...
    if (text?.length >= 6) {
      dispatch(
        editSetqrdataAction(Token, text, route.params?.item?.invoice_id),
      );
      setQrcode([]);
    }
  };
  ...
  useFocusEffect(
    React.useCallback(() => {
      dispatch(geteditqrdataAction(Token, route.params?.item?.invoice_id));
    }, []),
  );

  return (
    <>
      {billsubmitloading || restartloading || editqrloading ? (
        <View
          style={{
            marginTop: 'auto',
            marginBottom: 'auto',
          ...
          <ActivityIndicator
            animating={billsubmitloading || restartloading || editqrloading}
            color={'#9ECED9'}
            size={scale(30)}
          />
        </View>
          ...
          <View
            ...
            {deleteeditbillloading ? (
              <View
                ...
                />
                <ActivityIndicator
                  animating={deleteeditbillloading}
                  color={'#9ECED9'}
                  size={scale(30)}
                />
              </View>
                  ...
                  <Pressable
                    ...
                    <View
                      style={{
                        ...
                        shadowRadius: 4,
                        elevation: 7,
                        backgroundColor: liked.includes(index)
                          ? 'lightgrey'
                          : '#FFFFFF',
                      ...
                      <View
                        ...
                        <Pressable
                          ...
                          onPress={() => {
                            navigation.navigate('editBillProductUpadte', {
                              price: item.price,
                              pieces: item.pieces,
                              qty: item?.qty,
                              editid: item?.id,
